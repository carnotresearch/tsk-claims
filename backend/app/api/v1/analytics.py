...
async def kpis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch KPIs analytics")
...
async def tat_stats(
    ...
    try:
        ...
        logger.error("Failed to compute TAT stats: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch TAT statistics")
...
async def tat_detailed(
    ...
    try:
        ...
        logger.error("Failed to compute detailed TAT: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch detailed TAT analytics")
...
async def ageing(
    ...
    try:
        ...
        logger.error("Failed to compute ageing: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch ageing analytics")
...
async def ageing_by_payer(
    ...
    try:
        ...
        # Dynamically include standard payers first, followed by all other distinct payers found in DB
        standard_order = ["TPA", "Insurer", "Govt Scheme", "Self-funded"]
        all_payers: list[str] = []
        for p in standard_order:
            all_payers.append(p)
        for r in rows:
            if r.payer_type and r.payer_type not in all_payers:
                all_payers.append(r.payer_type)
        ...
        logger.error("Failed to compute ageing by payer: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch ageing by payer analytics")
...
async def top_disallowances(
    ...
    try:
        # Join to Claim and apply hospital scope filter to prevent cross-hospital data leak
        q_denials = (
            select(
            ...
            .join(Claim, QueryDenial.claim_id == Claim.id)
            .where(QueryDenial.disallowed_reason.isnot(None))
        )
        q_denials = _scope_filter(q_denials, scope, hospital_id)
        q_denials = q_denials.group_by(QueryDenial.disallowed_reason)
        ...
        # Append any other distinct reasons from DB that did not match standard categories
        for k, v in reason_map.items():
            if k not in matched_reasons:
                result.append(
                    DisallowanceReasonRow(
                        reason=k,
                        cases_count=v.cases_count,
                        disallowed_amt=Decimal(str(v.disallowed_amt)),
                    )
                )

        return result
    except Exception as exc:
        logger.error("Failed to compute top disallowances: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch disallowances analytics")
...
async def status_snapshot(
    ...
    try:
        ...
        logger.error("Failed to compute status snapshot: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch status snapshot analytics")
...
async def payer_performance(
    ...
    try:
        ...
        # Dynamically include standard payers first, followed by all other distinct payers found in DB
        standard_order = ["TPA", "Insurer", "Government", "Self-funded"]
        all_payers: list[str] = []
        for p in standard_order:
            all_payers.append(p)
        for r in rows:
            mapped_name = "Government" if r.payer_type in ("Govt Scheme", "Govt") else r.payer_type
            if mapped_name and mapped_name not in all_payers:
                all_payers.append(mapped_name)
        ...
        logger.error("Failed to compute payer performance: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch payer performance analytics")
...
async def monthly_trend(
    ...
    try:
        ...
        logger.error("Failed to compute monthly trend: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch monthly trend analytics")
...
async def monthly_detailed(
    ...
    try:
        ...
        logger.error("Failed to compute detailed monthly stats: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch detailed monthly analytics")
...
async def status_breakdown(
    ...
    try:
        ...
        logger.error("Failed to compute status breakdown: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to fetch status breakdown analytics")
